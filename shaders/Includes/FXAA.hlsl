...
To use FXAA,
  ...
  color.rgb = ToneMap(color.rgb);  // linear color output
  color.rgb = sqrt(color.rgb);     // gamma 2.0 color output (ideally use gamma 2.2 or even 3.0 etc)
  color.a = dot(color.rgb, FxaaFloat3(0.299, 0.587, 0.114)); // compute luma (this is BT.601, use BT.709 for modern color spaces)
  return color;
...
FxaaFloat4 FxaaPixelShader(
    //
    // Use noperspective interpolation here (turn off perspective interpolation). In UV 0-1 space, not pixel space!
    // {xy} = center of pixel
    FxaaFloat2 pos,
    //
    ...
    // {xy__} = upper left of pixel
    // {__zw} = lower right of pixel
    FxaaFloat4 fxaaConsolePosPos /*= FxaaFloat4(0, 0, 0, 0)*/,
    //
    // Input color texture.
    ...
    FxaaTex fxaaConsole360TexExpBiasNegOne /*= tex*/,
    //
    // Only used on the optimized 360 version of FXAA Console.
    // For everything but 360, just use the same input here as for "tex".
    // For 360, same texture, just alias with a 3nd sampler.
    // This sampler needs to have an exponent bias of -2.
    FxaaTex fxaaConsole360TexExpBiasNegTwo /*= tex*/,
    //
    ...
    FxaaFloat4 fxaaConsoleRcpFrameOpt = FxaaFloat4(0.0, 0.0, 0.0, 0.0),
    //
    // Only used on FXAA Console.
    ...
    FxaaFloat4 fxaaConsoleRcpFrameOpt2 = FxaaFloat4(0.0, 0.0, 0.0, 0.0),
    //
    // Only used on FXAA Console.
    ...
    FxaaFloat4 fxaaConsole360RcpFrameOpt2 = FxaaFloat4(0.0, 0.0, 0.0, 0.0),
    //
    // Only used on FXAA Quality.
    ...
    FxaaFloat fxaaConsoleEdgeSharpness = FxaaFloat(8.0),
    //
    // Only used on FXAA Console.
    ...
    //   0.25 leaves more aliasing, and is sharper
    FxaaFloat fxaaConsoleEdgeThreshold = FxaaFloat(0.125),
    //
    // Only used on FXAA Console.
    ...
    //   Tune by looking at mostly non-green content,
    //   then start at zero and increase until aliasing is a problem.
    FxaaFloat fxaaConsoleEdgeThresholdMin = FxaaFloat(0.05),
    //    
